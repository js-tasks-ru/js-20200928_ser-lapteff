...
export default class SortableTable {
  ...
  constructor(headersConfig, {
    ...
    // console.log(this.element.subElements)
    this.render();
    // this.getRequest();
  }
  async getRequest(id, order, start, end) {
    // https://course-js.javascript.ru/api/rest/products?_embed=subcategory.category&_sort=title&_order=asc&_start=0&_end=30
    const apiUrl = '/api/rest/products';
    const url = new URL(apiUrl, BACKEND_URL);
  ...
  onWindowScroll = async () => {
    ...
    if (bottom < document.documentElement.clientHeight && !this.loading && !this.isSortLocally) {
      ...
      const data = await this.getRequest(id, order, this.start, this.end);
  ...
  async render() {
    ...
    // const sortedData = this.sortData(id, order, this.getRequest());
    const data = await this.getRequest(id, order, this.start, this.end);
  ...
  initEventListeners() {
    this.subElements.header.addEventListener('pointerdown', this.onSortingTable);
  ...
  async sortOnServer(id, order, start, end) {
    const data = await this.getRequest(id, order, start, end);
